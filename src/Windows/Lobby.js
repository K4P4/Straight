...
class Lobby extends Component {
        ...
        <tbody>
          {this.state.data.players.map((user, index) => {
            return (
              <tr key={index}>
                <th>{user.isHost ? "👑" : ""}</th>
                <th className = "username">{user._id.username}</th>
  ...
  renderButtonMenu() {
    return (
      <div>
        <button className = "leave" onClick={this.leaveLobby}>Leave</button>
        {this.state.isHost && <button className = "start" onClick={this.startGame}>Start</button>}
      </div>
    );
  }
        ...
        case 0:
          return (
            <div className = "container">
              <h1 className = "lobbyName">Lobby {this.state.data.name}</h1>
              {this.renderTable()}
